...
def get_dynamic_list(uid):
    ...
    try:
        ...
        print('[{}]{}'.format(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), resp_json['data'])[0:120])
...
def compare_dynamic(dynamic1, dynamic2):
    if dynamic1 and dynamic2:
        desc1 = dynamic1['desc']
        desc2 = dynamic2['desc']
        dynamic_id1 = desc1['dynamic_id']
        dynamic_id2 = desc2['dynamic_id']
        timestamp1 = desc1['timestamp']
        timestamp2 = desc2['timestamp']
        if dynamic_id1 != dynamic_id2 or timestamp1 != timestamp2:
            return True
    return False
        ...
        try:
            ...
            if '10:00:00' < now < '24:00:00':
                ...
                if compare_dynamic(old_dynamic, new_dynamic):
                    ...
                    type, oid = check_dynamic(new_dynamic)
